...
**WP-8 · Kinds on the surface, and the worker** (W) ✅ **done 3 Sep 2026** — `Demos/surface/13-kinds.js`: every textual kind renders into the same script-less, same-origin iframe with `data-region` on what ink lands on (a script's functions, data's keys, prose's headings, a vector's elements), so `regionsUnderInk` reads a script as it reads a page. The worker is built from a string (one file, standalone-safe); a `js` artifact's code is loaded and stepped only while its clock plays, and play is the human's event (I9). A throw pauses the clock with the reason; a step past `RUN_BUDGET_MS` terminates the worker and pauses with the budget named; the frame is marked broken; the board goes on drawing (e2e 19–19g). Engine: `code` events carry a `kind`; the `clock` event and `state.clocks` landed here rather than in WP-5, because the bless-to-run gate *is* a clock. *Contract note:* `clocks` holds `{playing, seed, at, reason?}` — time itself is runtime, derived by the surface, never in the log. What moves for now is the artifact's own frame; instances are WP-5.
...
**WP-6 · Words into verbs, and acting it out** (W, needs WP-4, WP-5) ✅ **done 3 Sep 2026** — `src/behave/words.ts`: a table of the ways each verb is said (longest phrase wins; targets are the nouns the clause points at, singular; size and pace are modifiers), so the common phrasing needs no model; what the table cannot read is returned, not dropped. `agent.behave` asks a model only for those clauses, against the closed verb list, and holds the result attributed. The `behave` event: a human's behaviour is blessed by the act, a model's or the fit's is held until a human gives it in their name; the tank runs the newest blessed one. Surface: words typed at a definition in the loop become its behaviour on the spot; a label's transcript that reads as verbs is offered; held offers sit in the palette and the panel with *use it*; dragging a body while its clock runs is a demonstration, fitted onto the basis at the pace it was shown and held with each term's share and the residual named. The panel's ladder: words → sliders (a term's weight, one event on release) → what each verb is doing now → source under the js contract. e2e 21–21d. *Found:* the fit modelled the dragged body at thumbnail size, so every range-relative verb saw nothing in reach — it now takes the body's own size; and a demonstration faster than the verbs' speed reads as every verb pulling back, so the fit is made at the pace shown and the behaviour keeps that speed.
...
**WP-10 · Frames** (W, needs WP-7, WP-8, WP-12) ✅ **done 3 Sep 2026** — `src/frames/frame.ts` and `Demos/surface/16-frames.js`. **The drawn slider**: a line with a dot on it is the `slider` concept; *Make it a slider* makes a `control` artifact whose value is where the knob sits along the track, read from the ink where it stands — so dragging the knob (no selection needed, one `move` on release) *is* setting the value. **Interfaces are read, not declared**: a script's tunables are its top-level numeric constants, a page's slots its regions, a control's port its value, a word's its text, a behaviour's its speed and weights. **Connections** are offered by type, ranked by name, with reasoning; the `frame` event makes an artifact that *refers* to its members (nothing moves) and carries the wiring; **resolution** substitutes the wired values where members render (the iframe), run (the worker) and step (the tank). Export writes the wired files plus `frame.json` and an `index.html` when a page is among them. **Conjuring**: a frame built once is offered to a loop by the name written beside it or by resemblance (the same kinds of member), and applying it maps the template's ports onto the new members. e2e 22–22i. *Deferred:* a frame whose `js` transforms a selection into a new artifact (the SNA mapper's code) runs through the same worker contract but is not yet a palette verb; rewiring an existing frame is a new `frame` event.
...
**WP-11 · The folder** (W, needs WP-1) ✅ **done 3 Sep 2026** — surface half in `Demos/surface/17-folder.js`: *Open a folder…* (File System Access) or `?folder=<base>` (a static site with a manifest); the merged logs are loaded, then every file of a known kind not yet on the board becomes an `import` event in this participant's log, laid out as cards, so a second machine that pulls sees the same board and discovers nothing twice (e2e 23–23b). Autosave rewrites this participant's own log file after every change, or holds the whole log in browser storage when there is no folder — a reload brings the board back; *Reset* forgets it. The live budget: the nearest twelve live artifacts render, the rest stand as parked cards and the status says so (23c). Grid and focus views are lenses over the same log (23d). *Found:* the tank and worker loops relied on animation frames, and a tab the browser stops painting gets none — time is state here, so the loops take a timer's tick when no frame comes (`nextFrame`), and the e2e steps the worker by hand as it does the tank. *Honest gaps:* snapshots of parked artifacts are placeholder cards, not pictures (an iframe cannot be rasterised from outside); a folder open on a phone needs WP-14's git-backed store; the walk skips `node_modules` and stops at 400 files, saying so. **Engine half**: `src/store/seam.ts` (the `Store` interface per 1.5, `Entry`, the log layout `.metamedium/logs/<participant>.jsonl`, one event per line, `MemoryStore`), `src/store/static.ts` (read-only, discovered through `.metamedium/manifest.json`, so a published canvas can be drawn on and the ink stays the reader's), `src/store/folder.ts` (File System Access, typed structurally so core carries no DOM typings and a fake folder drives the tests). A read-only backend refuses with a `ReadOnlyError` naming what was refused.
...
**WP-9b · Images on the surface** (W, needs WP-9a, WP-11) ✅ **done 3 Sep 2026** — `Demos/surface/18-images.js`: a picture arrives by drop, paste, *Import…* or a phone's camera (`capture`), is downsampled, traced, and lands as declared ink at the drop point with the raster kept beside it as an image artifact (and in the folder's `imports/` when there is one to write to); an SVG or any file of a known kind becomes an artifact of its kind. The e2e paints a "photo" of three boxes, traces it into three rectangles, circles them and prompts a page that renders inside its own ink (24–24b). Export: the board as SVG (one path per stroke, the clean form where one is held, each path naming its node and its reading) or PNG, the session as its log, and any artifact's code from the panel. *Deferred:* SAM on request.
