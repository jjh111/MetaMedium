...
| `src/view.ts` | **Pure.** The camera's own arithmetic: the six axis views with the up that makes each named plane read in its own frame, the flip to the other side, `viewFacingPlane` / `planeFacedBy` (the compass and the plane picker agreeing), `axisPlaneFor` and `planeAfterLeavingAxisView` (the axis view IS the choice, and what becomes of it on the way out — one line, so the other reading is one edit), `isAxisView` / `AXIS_VIEW_TOLERANCE_DEG`, `tooOblique` against the scorer's own `FACING_FLOOR`, `frameFor` (a bounds → a pose that fits it, sphere not box, at any aspect, in persp and in ortho), `orthoHeightFor` / `distForOrthoHeight` (why the projection toggle does not jump), and `balls` — the six axes projected onto the camera's screen basis, farthest first. No three.js |
| `src/navgizmo.ts` | The compass in the corner, as an **SVG overlay** built from the tokens: three arms from a centre with a labelled ball on each positive end and a hollow one on each negative, depth-sorted and turning with the camera, tappable (snap, and flip on a second tap), draggable (orbit, one finger, because it is chrome rather than canvas) — plus the *home* and *view* tiles and the pinned-view chips. It also watches the camera cross in and out of an axis view and tells the surface, which is what couples it to the picker (`enterAxisView` / `leaveAxisView`, `standing`) |
| `src/gizmo.ts` | The three axes, the three tiles, the slide handle, the centre — all of it standing at **the cursor**, whose planes it hands out through that point (`origin` / `setOrigin`). The tiles and the handle hide in an axis view (`showTiles`), leaving the cursor mark and its axes; `chosenWhy` carries why this plane is the chosen one, so ink chosen by the camera does not claim a tile |
...
| `e2e.js` | The whole loop through the real pointer path — **95 steps, P0 → P6 and the compass** — and, beside it, `__demo()`: the two-minute demo of §9 in ten asserted steps, with a timing on each |
