...
| `castle-sketch.ideal.json` | `?demo=castle-sketch` | parts | Written by hand — what a perfect answer looks like. Every part named from the human's own words, one colour word each, two small ops by part id. Show this to a model that is drifting. **It is also the demo's own reply**: `?demo=castle` and `__demo2()` seat a stub that answers with this exact text, and `src/demo.test.ts` holds the two against each other so neither can drift (the two boards are the same board — `?demo=castle` is John's castle sketch, drawn beat by beat) |
