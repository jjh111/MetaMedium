...
**Status — every package landed 16 September 2026**, on the branch
`control-points` and then G4 after it. The way `SURFACE-v10-PLAN.md` carries
status: one dated line per package, with the one finding it produced.

| # | Landed | The finding it produced |
|---|---|---|
| G0 | 16 Sep 2026 | The board goes out and comes back as **its own core-format log** — the canvas's `encodeLog`, unchanged — so John's boards became fixtures and this plan's claims are settled on a real drawing. `?fixture=john-2026-09-16-massing` stands his three profiles at **y ∈ [0.97, 3.09]**: the massing was never in the floor, and the free *ink* was. The panel's *model* section is the transcript (the brief as sent, the reply as received, verbatim), and `briefTarget()` means a brief is never refused for want of a selection |
| G5 | 16 Sep 2026 | **The seat is a model, and that is the whole of it.** A seat carries an injectable `transport` — `bridge.ts`'s pattern — so `runBrief` needs no case for the hand: the prompts, the parser, the dropped-and-counted rule, the work dot and Esc are all unchanged, and only *where the question goes* differs. The brief rides the log as an answer on the explanation plane, with the pairing in the payload because node ids are per hand. Found at once: `space_look` must read every node carrying ink and a plane, not `contentIds` |
| G1 | 16 Sep 2026 | **A hull occupies the volume its claims define**, and each claim's prism runs through the span of the *others* — a footprint's own points all sit at y = 0, and reading them into its own vertical span is how the ground gets into a body no claim's feet reach. And the real fault of John's second board was the **cursor**, which had never left the world origin while he looked one to four units up: the view plane follows the centre of the view now, which is the one place the shard parts company with Blender |
| G2 | 16 Sep 2026 | **One standpoint is one silhouette.** Two towers drawn from one place are one outline with two pieces, not two claims to intersect — intersecting them gave the empty set, which is what John's board came to. Claims sharing a plane direction are gathered first (apart → unioned, overlapping → intersected). And the honest limit, measured rather than worked around: his board gives **two parts, not three**, because a tower seen once has no depth |
| G3 | 16 Sep 2026 | **The brief takes a second shape and the reply a second contract**, and one function (`partIdsOf`) decides which, so the brief, the prompt, the parser and the landing can never each decide differently. John's castle is **1048 characters** before `HERE_ON_A_HULL`. A part's name lives on the hull step keyed by **the claims it was cut from**, never by `part:n`, because a dropped claim renumbers everything after it. qwen3:8b answered in 42 s with strict JSON first time, kept verbatim in `fixtures/exchanges/`. **Still owed:** glm-flash through John's own key |
| G4 | 16 Sep 2026 | **What is shown and what is proved are one board.** `?demo=castle` and `__demo2()` read the same numbers, and John's two boards are logs now (`fixtures/make.mjs`). Three faults the beats exposed: the panel's parts wiring silently dropped every part's `name` and `colour`, so a chip never showed what G3 had named; `splitPrompt` knew only the steps contract's marker, so on a hull — the board this whole plan is about — the hand was handed the brief with the human's words stripped out; and the free loop of beat 4 is itself taken as a claim, which is right and means the test must put the board back in the two acts it took to change it |
...
| G4 ✅ | **The loop on John's own drawing** — *landed 16 Sep 2026* | John's board exported from the log as a fixture; the demo re-cut: footprint → walk around → towers from two views → the hull stands → the brief → names and green → *why* → *name: castle* → the footprint drawn again is offered as castle; the e2e drives it from the recorded strokes | the fixture runs green in the headless gate. **Done**: both of John's boards are real core-format logs (`fixtures/make.mjs`), so `?fixture=` and *Open…* load the same bytes; `?demo=castle` is the eight-beat loop with `__demo2()` asserting each beat from the same numbers (`src/demo.ts`), and a ninth optional beat runs the same path through the MCP seat; `e2e/run.mjs` gained a fourth scenario, and all four are green — **342 records, one honest skip, 111 s**; and `shard-3d/README.md` is one document again |

Order, as built: G0 → G5 → G1 → G2 → G3 → G4, all of it on 16 September 2026. G0 is small and first because it is
